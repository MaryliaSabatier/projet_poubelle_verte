<?php
// Définir les arrêts avec leurs coordonnées géographiques
$stops = array(
    // Rue Croix-Baragnon
...
// Fonction pour obtenir les intersections entre les rues
function getIntersections($streets) {
    $stop_streets = array();
    foreach ($streets as $street => $stops) {
        foreach ($stops as $stop) {
            if (!isset($stop_streets[$stop])) {
                $stop_streets[$stop] = array();
            }
            $stop_streets[$stop][] = $street;
        }
    }

    $intersections = array();
    foreach ($stop_streets as $stop => $streets_at_stop) {
        if (count($streets_at_stop) > 1) {
            $intersections[$stop] = $streets_at_stop;
        }
    }
    return $intersections;
}

$intersections = getIntersections($streets);
?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8" />
    <title>Visualisation des Rues et Arrêts</title>
...
<h1>Visualisation des Rues et Arrêts</h1>
<div id="map"></div>
...
<script>
// Initialiser la carte sans couche de tuiles
var map = L.map('map', {
...
// Ajouter un fond blanc à la carte
L.tileLayer('', {
    attribution: '',
    maxZoom: 20,
    minZoom: 1
...
// Données des rues depuis PHP
var streets = <?php echo json_encode($streets); ?>;

// Fonction pour choisir une couleur pour chaque rue
...
// Ajouter les rues à la carte
for (var street in streets) {
    var streetStops = streets[street];
    var latlngs = [];
    for (var i = 0; i < streetStops.length; i++) {
        var stopName = streetStops[i];
        if (stops[stopName]) {
            latlngs.push([stops[stopName].lat, stops[stopName].lng]);
        }
    }
    var polyline = L.polyline(latlngs, {color: getRandomColor()}).addTo(map);
    polyline.bindPopup("<b>" + street + "</b>");
}

// Ajouter les intersections à la carte
var intersections = <?php echo json_encode($intersections); ?>;
for (var stop in intersections) {
    var intersectionMarker = L.circleMarker([stops[stop].lat, stops[stop].lng], {
        radius: 8,
        fillColor: '#FF0000',
        color: '#FF0000',
        weight: 2,
    ...
    intersectionMarker.bindPopup("<b>Intersection: " + stop + "</b><br>Rues: " + intersections[stop].join(', '));
}
</script>
